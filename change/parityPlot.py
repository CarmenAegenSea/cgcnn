"""
绘制 CGCNN 带隙预测结果图（修复转义警告版本）

输入：CGCNN 测试集预测结果 CSV（包含 target 和 prediction 列）
输出：散点图、误差分布直方图、损失曲线（若提供日志）
"""
...
# ==================== 数据准备 ====================
# 方式1：从实际 CSV 读取（请替换为你的文件路径）
# df = pd.read_csv("test_results.csv")
# y_true = df["target"].values
# y_pred = df["prediction"].values

# 方式2：模拟数据（根据你的 MAE 0.366 eV 生成）
np.random.seed(42)
n = 100
y_true = np.random.uniform(1.0, 3.0, n)
noise = np.random.normal(0, 0.366, n)
y_pred = y_true + noise
