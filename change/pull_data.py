...
脚本功能：
1. 定义过渡金属(38种)和硫族元素(S, Se, Te)
2. 生成所有可能的二元化学体系组合(过渡金属-硫族元素)
3. 对每个组合查询Materials Project，获取材料数据
4. 支持下载CIF晶体结构文件
5. 保存为CSV/JSON格式，便于后续分析
"""
...
def generate_chemsys_combinations(t_metals: List[str], chalcogens: List[str]) -> List[str]:
    """
    生成所有过渡金属与硫族元素的二元化学体系组合

    Returns:
        List[str]: 如 ["Sc-S", "Sc-Se", "Sc-Te", "Ti-S", ...]
    """
    combinations = []
