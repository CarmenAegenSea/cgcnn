...
# 获取当前脚本所在目录（.../cgcnn/change/）
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
# 项目根目录：脚本目录的上上级？不，因为脚本在 cgcnn/change/ 下，所以上一级是 cgcnn/，这就是包含 data 的根目录
PROJECT_ROOT = os.path.dirname(SCRIPT_DIR)  # 得到 .../cgcnn/cgcnn
...
for i in range(1, 6):
    ...
    if not os.path.isdir(fold_dir):
        print(f"⚠️ 警告：文件夹 {fold_dir} 不存在，跳过。")
        continue

    cif_files = [f[:-4] for f in os.listdir(fold_dir) if f.endswith('.cif')]
    if not cif_files:
        print(f"⚠️ 警告：{fold_dir} 中没有 .cif 文件，跳过。")
        continue
